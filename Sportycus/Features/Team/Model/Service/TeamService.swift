...
protocol TeamServiceProtocol {
    func getTeam(completion: @escaping (TeamResponse?) -> Void, sport: String, teamName: String, leagueId: Int)
}
...
class TeamService : TeamServiceProtocol {
    ...
    func getTeam(completion:@escaping (TeamResponse?) -> Void, sport: String, teamName: String, leagueId: Int) {
        ...
        let url = "\(baseUrl)\(sport)?met=Teams&leagueId=\(leagueId)&teamName=\(teamName)&APIkey=\(apiKey)"
